...
#  @修改时间    : 2023-11-23 10:09:55
#  @上次修改    : 2023/11/23 下午10:09
import json
import os.path
import time
...
@app.get("/getNewCookie")
def cookies():
    from flaskSystem.src.Common import Http
    from flaskSystem.API.qq import QQApi
    GitCode=Http.HttpRequest()
    print("准备开始获取Cookie")
    d = GitCode.getHttp('https://gitcode.net/qq_29574377/project/-/raw/master/Update_bobiyinyue.json?inline=false').text
    b = json.loads(d.encode('utf8')[3:].decode('utf8')).get('cookie')
    #qqcookie = b.get('cookie')
    print(b)
    QQApi.setQQCookie(b)
    return {
        'code': 200,
