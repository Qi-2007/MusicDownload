...
#  @修改时间    : 2023-11-23 08:36:08
#  @上次修改    : 2023/11/23 下午8:36
import base64
import hashlib
import json
import os
...
class Netease(BaseApi):
    __baseUrl = 'https://api.lxtlovely.top'
    ...
    def cloudmusic_dll_encode_id(self, some_id):
        ID_XOR_KEY_1 = b'3go8&$833h0k(2)2'
        xored = bytearray(
            [char ^ ID_XOR_KEY_1[idx % len(ID_XOR_KEY_1)] for idx, char in enumerate(some_id.encode())]
        )
    ...
    def anonimousLogin(self):
        os = 'iOS'
        device_id = "NMUSIC"
        encoded_id = f"{device_id} {self.cloudmusic_dll_encode_id(device_id)}".encode()
        username = base64.b64encode(encoded_id).decode()
        u = "https://music.163.com/weapi/register/anonimous"
        res = self.httpwj(u, 1, weapi('{"username":"' + username + '","csrf_token":""}'))
        res = res.json()
        return res
    ...
    def qrLogin(self):
        u = 'https://music.163.com/weapi/login/qrcode/unikey'
        res = self.httpwj(u, 1, eapi(u, {
            'type': 1,
        }))
    ...
    def searchMusic(self, searchKey="周杰伦", pageNum=0, pageSize=100):
        """
        官方接口搜索网易云歌曲
        Args:
            searchKey:
