...
import {decrypt, eapi, linuxapi, weapi} from "./crypto";
import crypto from "crypto";


export const chooseUserAgent = () => {
    const userAgentList = {
        mobile: [
    ...
    let realUserAgentList = userAgentList.mobile.concat(userAgentList.pc);
    return realUserAgentList[0];
    return realUserAgentList[
        Math.floor(Math.random() * realUserAgentList.length)
        ];
};
export const GenerateApi = (
    ...
    let headers: any = {"User-Agent": chooseUserAgent()};
    ...
    // 发送完毕后准备接受数据
    return Api.CallHttpApiGetResponse(
        "/common/getHttp",
        1,
        ...
        try {
            if (options.crypto === "eapi") {
                answer.body = JSON.parse(decrypt(body).toString());
            } else {
                answer.body = body;
            }
            if (answer.body.code) {
